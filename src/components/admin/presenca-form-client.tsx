...
function primeiroEncontroDoModulo(encontros: EncontroOpt[], moduloNumero: number): string | null {
  const list = encontros
    .filter((e) => e.moduloNumero === moduloNumero)
...
export function PresencaFormClient({
  ...
  useEffect(() => {
    if (modulosResolver.length === 0 || encontros.length === 0) {
      setEncontroId("");
      return;
    }
    const mod = resolverModuloParaData(modulosResolver, dataRef);
    if (!mod) return;
    const sug = primeiroEncontroDoModulo(encontros, mod.numero);
    if (sug) setEncontroId(sug);
  }, [dataRef, modulosResolver, encontros]);
  ...
  const modSugerido = modulosResolver.length
    ? resolverModuloParaData(modulosResolver, dataRef)
    : null;
  ...
  return (
    <form action={action} className="space-y-6">
        ...
        <label className="block md:col-span-2">
          ...
          {modSugerido ? (
            <p className="mt-1 text-xs text-zinc-500">
              Módulo sugerido pela data:{" "}
              <strong className="text-zinc-700">
                {modSugerido.codigoPublico ?? `Módulo ${modSugerido.numero}`}
              </strong>
              {modSugerido.mesReferencia && modSugerido.anoReferencia
                ? ` (${String(modSugerido.mesReferencia).padStart(2, "0")}/${modSugerido.anoReferencia})`
                : null}
            </p>
          ) : null}
          <select
      ...
      <button
        type="submit"
        disabled={pending || !turmaId || alunos.length === 0 || !encontroId}
